...
<script type="text/javascript">
    ...
    $(function () {
        ...
        initMap();
        init();
        search($("#address").val(),0);
        sc = $("#shopCode").val();

        $("#scity").change(function () {
            $("[id='city.id']").val($("#scity").val());
        });
        $("[id='city.id']").val($("#scity").val());
...
//        map.setCurrentCity("上海");   // 仅当设置城市信息时，MapTypeControl的切换功能才能可用
    ...
    function checkShopCode(){
        ...
        if (code == sc) return;
        var id = $("#id").val();
        var orgId = $("[id='organization.id']").val()
        ...
        murl = murl + code + "&id=" + id + "&orgId=" + orgId;
        $.ajax({
            url:murl,
    ...
    function getUrl(key){
        ...
        if(key == ''){
            ...
            url = "http://upload.carmore.cc/img/" + key;
...
<div class="row" style="margin-top: -170px;">
    <div class="col-md-5 col-md-offset-2">
        <div id="actions" class="form-action">
            <form id="fm" action='<@spring.url relativeUrl = "/shop/new"/>' method="post">
                ...
                <#if pageContent?? && pageContent == "更新">
                    <legend>门店管理 -> 更新门店</legend>
                <#else>
                    <legend>门店管理 -> 新增门店</legend>
                </#if>
                ...
                <div class="row">
                    ...
                    <div class="col-md-5 col-md-offset-2">
                        <@form.textInput "shop.city.id" "class='form-control'" "hidden" "所在城市" true/>
                        <select id="scity" class="form-control select2">
                            <#list citys as city>
                                <#if shop.city?? && shop.city.id == city.id>
                ...
                <div class="row">
                    <div class="col-md-5">
                        <br/>
                        <@form.textInput "shop.address" "class='form-control' style='width:615px' onblur='search(this.value,0);'"  "text" "门店地址" true/>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-5">
                        ...
                        <#--<span>上传营业执照</span>-->
                        <input id="fileupload" type="file"   name="fileUpload" data-url="http://upload.carmore.cc/upload/"   multiple>
