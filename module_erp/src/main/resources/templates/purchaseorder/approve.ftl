...
<script type="text/javascript">
    ...
    $(function () {
        ...
        <#if (purchaseOrder.purchaseOrderDetailList)?? >
            <#list purchaseOrder.purchaseOrderDetailList as rowData >
                ...
                "customStockItem.name":"${rowData.customStockItem.name!}", //商品名称
                    ...
                    "lastPrice":${rowData.lastPrice?c!0}, //上次进货价格
                "bankNumber":${rowData.bankNumber?c!0}, //库存
        ...
        <#if viewStatus?? &&  viewStatus == "detail" >
    ...
    function passApprove(){
        window.location = "/purchaseorder/${indexPage}/result/1/" + $("#purchaseOrderId").val()+"?remark=" + $("#remark").val();
    }
    //退回方法
    function returnApprove(){
        var url = "";
        if ($("#remark").val() == "") {
            url = "/purchaseorder/${indexPage}/result/2/" + $("#purchaseOrderId").val();
        } else {
            url = "/purchaseorder/${indexPage}/result/2/" + $("#purchaseOrderId").val() + "?remark=" + $("#remark").val();
        }
        window.location  = encodeURI(url,"UTF-8");

    }
    function returnAddstorage(){
        var url =  "/purchaseorder/${indexPage}/result/2/" + $("#purchaseOrderId").val() + "?remark=" + $("#remark").val();
        window.location  = encodeURI(url,"UTF-8");
    }
    //入库方法
    function addStorage(){
        var url = "";
        if ($("#remark").val() == "") {
            url = "/purchaseorder/${indexPage}/result/3/" + $("#purchaseOrderId").val();
        } else {
            url = "/purchaseorder/${indexPage}/result/3/" + $("#purchaseOrderId").val() + "?remark=" + $("#remark").val();
        }
        window.location  = encodeURI(url,"UTF-8");
    }
    //返回list页
    function goToList(){
        ...
        window.location = "/purchaseorder/${indexPage}/list";
    }
    /*//打印页面
    function printHtml(){
        ...
        <div class="col-md-11">
            <div class="row">
                <div class="col-md-2 col-md-offset-3">
                    ...
                    <input value="${purchaseOrder.purchaseShop.id?c}" name="purchaseShop.id" id="shopId" type="hidden" >
                    <input value="${purchaseOrder.id?c}" id="purchaseOrderId" hidden >
                </div>
            ...
            <#--<div class="col-md-5">
                ...
                <div class="col-md-2 col-md-offset-3">
                    <@form.textInput "purchaseOrder.saleShop.name" "class='form-control' readonly" "text" "审批门店" />
                </div>
                <div class="col-md-2 col-md-offset-1">
            ...
            <div class="span2 text-center">
                <#--<#if pageContent?? && pageContent == "update">-->
                    <#if viewStatus?? && viewStatus == "approve">
                        ...
                        <@form.btn_back "onclick='returnApprove()' style='margin-left:30px'" "退回申请"/>
                    <#elseif viewStatus?? &&  viewStatus == "addstorage" >
                        <@form.btn_add "onclick='addStorage()'" "确认核实入库"/>
                        <@form.btn_back "onclick='setAuth()' style='margin-left:30px'" "退回申请"/>
                    <#else >
                    ...
                    <@form.btn_back "onclick='goToList()' style='margin-left:30px'"  "返回上一页"/>
