        ...
        function showList(url) {
            ...
            $("#shopBody").jqGrid({
                ...
                colModel: [
                    ...
                    { label: '库存数量', name: 'oldNumber', width: 50, align:"center"},
                    { label: '盘后数量', name: 'calculateNumber', width: 40 ,align:"center",
                        formatter:function(cellvalue, options, rowObject){
                              return  "<input type='text' "+ status +" class='input-sm form-control' onchange='showBeforeSum();' value='" + cellvalue + "' id='pNum_"+ rowObject["id"] +"'/>";
                        }
//
                    },
...
//
        ...
        function subForm(type,msg){
            var oneData ="";
            var obj = $("#shopBody").jqGrid("getRowData");
            var i = 1;
            jQuery(obj).each(function(){
                oneData += "" + this['id'] + ","+ $("#pNum_"+ i).val() + ";";
            });

            $("#listData").val(oneData);
            if(confirm("是否确认 "+msg+"! ")){
//                $("#stockingStatus").val(type);
        ...
        function blackPage(){
            window.location = "/stockingorder/list";
        }

        function exportExcel (){
            window.location = encodeURI("/stockingorder/excel/export?orderId=" + $("#id").val() + "&type=view" + "&shopId=" + $("#shopId").val(), "UTF-8");
        }
                ...
                <form class="" id="fm" action='<@spring.url relativeUrl = "/stockingorder/edit/save"/>'   method="post">
                    ...
                    <div class="col-md-8 col-md-offset-2">
                        ...
                        <div class="row" style="text-align: center">
                            ...
                            <@form.btn_back "onclick='blackPage();'" "返回上一页" />

                            <@form.btn_print "onclick='exportExcel()' align='center' style='margin-left:30px'" "数据导出" />

                            <#--<#if stockingOrder.stockingStatus == 0>-->
                                <#--<@form.btn_save "onclick='subForm(0,'保存盘点')'" "保存盘点信息"/>-->
                                <#--<@form.btn_calculator "onclick='subForm(1,'完成盘点')' style='margin-left:70px'" "结算完成盘点"/>-->
