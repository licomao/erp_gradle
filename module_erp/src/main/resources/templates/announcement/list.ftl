...
<#escape x as x?html>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            if (!${user.checkAuthority(AUTHORITY.MANAGE_ORG_NOTICE)?c}) {
            window.location = "/noauthority";
            } else {
                var url = "/announcement/list/data";
        ...
        function editById(id) {
            window.location = "/announcement/new?id=" + id;
        }
        function deleteById(id) {
            if(confirm("是否确认删除")){
                window.location = "/announcement/delete?id=" + id;
            }
        }

        function toAdd(){
            window.location ="/announcement/new";
        }
