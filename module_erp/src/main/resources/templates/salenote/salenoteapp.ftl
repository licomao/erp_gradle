...
//        }
        ...
        if(parseFloat(app) < parseFloat($("#sum").html())) {
            alert("成本价超出app总价");
            return;
        }
...
//            url: '/salenote/showOrderInfo',
            ...
            colModel: [
                ...
                { label: '实际售价(元)', name: 'receivable', width: 50,editable:true, hidden:true, align:"center",
                    formatter:function(cellvalue, options, rowObject) {
                        if(cellvalue == null ) {
                ...
                { label: '折后售价(元)', name: 'receivablediscount', width: 70, hidden:true, align:"center",
                    formatter:function(cellvalue, options, rowObject) {
                        if(cellvalue != null && cellvalue != "") {
                            return  '<input type="text" id="receivablediscount-' + rowObject['viewId'] + '-' + rowObject['cost'] + '" value="'+cellvalue+'" class="content"' +
                                    ' ' +
                                    'size="13" maxlength="10" onblur="changeDiscount('+options.rowId + ')" disabled="disabled" />';
                        }
                        return  '<input type="text" id="receivablediscount-' + rowObject['viewId'] + '-' + rowObject['cost'] + '" value="0.00" class="content"' +
                                ' ' +
                                'size="13" maxlength="10" onblur="changeDiscount('+options.rowId + ')" disabled="disabled" />';},
                },
                { label: '折扣率', name: 'discount', width: 60, align:"center", hidden:true},
                { label: '授权人', name: 'discountGranter', width: 60, hidden:true, align:"center",
                    formatter:function(cellvalue, options, rowObject) {
                        if(cellvalue != null && cellvalue != "") {
                ...
                { label: '总工时费', name: 'sum', width: 40,
                    formatter:function(cellvalue, options, rowObject) {
                        ...
                        return  '<input type="text" id="sum-'+rowObject['id'] +'" value="'+ costs +'" class="content"' +
                                ' ' +
                                'size="13" maxlength="15" style="text-align:center;" onblur="changeHourCost('+ options.rowId +')" readonly/>';}
                },
...
//                    {row: 3, widthStyle: "100%", outBorderStyle: "topAndBottom" }
            ...
            jqGridTables: [{
                ...
                labels: ['配件名称','配件性质','数量','单价','实际售价'],
                names: ['orderedItem.name','rootCategory','count','price','receivable'],
                jqGridId: 'customList',
//                headText: '商品列表',
                afterIndex: 4,
                outBorderStyle: "topAndBottom"
...
<SCRIPT type="text/javascript" FOR="myativx" EVENT="OnCapture(ActionResult ,ATemplate)">
  ...
  <legend>销售开单 -> 销售开单明细</legend>
      ...
      <form id="fm" class="" action='<@spring.url relativeUrl = "/salenote/saveapp"/>' method="post">
          ...
          <div class="col-md-6">
             ...
             <div class="row">
              ...
              <div class="row" style="margin-top: 1%">
                  ...
                  <div class="col-md-4">
                      <label class="control-label">上次保养里程：</label> <#if settleOrder.vehicleInfo.lastMaintenanceMileage??>${settleOrder.vehicleInfo.lastMaintenanceMileage}</#if>
                  </div>
                  <div class="col-md-4">
                      <label class="control-label">已行驶里程：</label> <#if settleOrder.vehicleInfo.mileage??>${settleOrder.vehicleInfo.mileage?c}</#if>
                  </div>
                  </div>
              </div>
                    <div class="row" style="margin-top: 1%">
  ...
  <div class="row" style="border:1px #000000 solid;padding-top: 0.5%">
      ...
      <div class="col-md-9">
          ...
          &nbsp; &nbsp; <label class="control-label">现金：</label> <input style="width:80px;" type="text" name="payment.cashAmount" id="payment.cashAmount" value="<#if settleOrder.payment??>${settleOrder.payment.cashAmount?c}<#else>0</#if>" readonly>
          ...
          &nbsp;&nbsp;<label class="control-label">App费用：</label> <input style="width:80px;"  type="text" name="payment.appAmount" id="payment.appAmount" value="<#if settleOrder.payment??><#if settleOrder.payment.amount gt 0>${settleOrder.payment.amount?c}<#else>${settleOrder.payment.appAmount?c}</#if><#else>0</#if>" readonly/>
    ...
    <div class=" col-md-12">
        <div clas="row">
            ...
            <div class="col-md-3">
                ...
                <#--<@form.btn_add "onclick='addup()'" "新增产品信息" />-->
            </div>
            <div class="col-md-5">
      ...
      <@form.textInput "settleOrder.presaleOrder.id" "" "hidden" />
    </form>
