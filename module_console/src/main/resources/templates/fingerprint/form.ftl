    ...
    <script type="text/javascript">
        ...
        function register() {
            try {
                ...
                var initResult = matx.InitEngine();
//                alert(initResult)
...
//                    $("#serial").val(matx.SensorSN);
                    ...
                    alert("错误代码: " + initResult + "。指纹识别驱动程序加载失败");
                    return ;
                } else if(initResult == 2){
                    alert("错误代码: " + initResult + "。没有连接指纹识别仪");
                    return ;
                } else if(initResult == 3){
                    alert("错误代码: " + initResult + "。属性 SensorIndex 指定的指纹仪不存在");
                    return ;
                } else {
                    alert("未识别的错误: " + initResult)
                    return ;
                }
...
//                var properties = computer.ExecQuery("SELECT * FROM Win32_USBHub");
    ...
    <div class="row" >
        ...
        <#--测试代码开始-->
            ...
            <@form.btn_search "onclick='register()'" "录入系统"/>&nbsp;
